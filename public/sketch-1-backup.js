...
body.innerHTML = '<button onclick="signIn();">Sign in</button><b><button onclick="createAccount();">Sign up</button><script src = "sketch.js" defer></script>';
...
function createAccount() {
	...
	firebase.auth()
		...
		.then((result) => {
			...
			var newAccountDataRef = database.ref('data');
			newAccountDataRef.push(newAccountData);
		}).catch((err) => {
			console.log(err);
		});
}
...
firebase.auth().onAuthStateChanged((user) => {
	if (user) {
		...
		// User is signed out
		// ...
		body.innerHTML = '<button onclick="signIn();">Sign in</button><br><button onclick="createAccount();">Sign up</button><script src = "sketch.js" defer></script>';
	}
});
